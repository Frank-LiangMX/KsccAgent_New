...
移植自 GenericAgent (https://github.com/lsdefine/GenericAgent)
"""
...
def _detect_tools():
    tools = [
        "git", "docker", "npm", "pip", "curl", "wget", "kubectl",
...
def format_results_text(results: list[SearchResult], query: str) -> str:
    """格式化搜索结果为可读文本"""
    if not results:
        return f"未找到匹配 \"{query}\" 的外部 Skill。试试其他英文关键词？"

    lines = [f"找到 {len(results)} 个匹配的外部 Skill：\n"]
    for i, r in enumerate(results, 1):
        s = r.skill
        safe_tag = "安全" if s.autonomous_safe else "需谨慎"
        lines.append(f"**{i}. {s.name}** [{safe_tag}]")
        if s.one_line_summary:
            lines.append(f"   {s.one_line_summary}")
        lines.append(f"   类别: {s.category} | 评分: {r.final_score:.2f}")
        if s.tags:
            lines.append(f"   标签: {', '.join(s.tags[:5])}")
        if r.warnings:
            for w in r.warnings:
                lines.append(f"   ⚠️ {w}")
        lines.append("")
